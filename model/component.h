...
namespace model {
	...
	class Component {
		...
		InputPin *get_input_pin(unsigned index) const;
		InputPin *get_input_pin(std::string name) const;
		const std::vector<OutputPin *> &output_pins(void) const;
		OutputPin *get_output_pin(unsigned index) const;
		OutputPin *get_output_pin(std::string name) const;
